...
import { blockMediaUrl } from '@/lib/media-url'
...
// 正文媒体块的 src/href：Notion 托管文件（.file.url）是 1 小时过期的 S3 直链 → 走 /api/media 稳定代理；
// 外链（.external.url，如 YouTube/Unsplash）不过期，直接用。代理 URL 用块的 last_edited_time 做版本键。
function blockMediaSrc(block, data) {
  if (data?.file?.url) return blockMediaUrl(block.id, block.last_edited_time)
...
function toSoundCloudEmbedUrl(url) {
  try {
    const u = new URL(url)
    if (u.hostname.toLowerCase() === 'w.soundcloud.com') return url
  } catch {}
...
function getSoundCloudPlayerHeight(url) {
  try {
    const u = new URL(url)
    return u.pathname.includes('/sets/') ? 450 : 166
  } catch {
    return 166
  }
}

function renderSoundCloudEmbed(url, caption = '') {
  const embedUrl = toSoundCloudEmbedUrl(url)
  const height = getSoundCloudPlayerHeight(url)
  const safeCaption = caption ? escapeHtml(caption) : ''

  return `<figure class="my-10">
    <div class="w-full overflow-hidden rounded-xl border border-border bg-card" style="height: ${height}px;">
      <iframe title="SoundCloud 音频播放器" src="${escapeAttr(embedUrl)}" class="w-full h-full border-0" loading="lazy" allow="autoplay"></iframe>
    </div>
    ${safeCaption ? `<figcaption class="text-center text-muted-foreground text-sm mt-3">${safeCaption}</figcaption>` : ''}
    <div class="mt-3 text-sm text-muted-foreground">
      <a href="${escapeAttr(url)}" target="_blank" rel="noopener noreferrer" class="text-primary hover:text-primary/80 underline decoration-primary/60 hover:decoration-primary transition-colors">在 SoundCloud 打开</a>
    </div>
  </figure>`
}

function getStandaloneLinkedUrl(richTextArray) {
  if (!richTextArray || richTextArray.length !== 1) return null
  const item = richTextArray[0]
  const href = item?.href
  const plainText = (item?.plain_text || '').trim()
  if (!href || !isSoundCloudUrl(href)) return null
  if (plainText === href || plainText.includes('soundcloud.com')) return href
  return null
}
...
function renderSingleBlock(block) {
  switch (block.type) {
    case 'paragraph': {
      ...
      const standaloneSoundCloudUrl = getStandaloneLinkedUrl(richText)
      const childHtml = block.children?.length ? renderBlocks(block.children) : ''
      if (standaloneSoundCloudUrl && !childHtml) return renderSoundCloudEmbed(standaloneSoundCloudUrl)
    ...
    case 'code': {
      ...
      return `<div data-copy-allow class="my-8 rounded-lg overflow-hidden border border-gray-800">
    ...
    case 'image': {
      ...
      const src = blockMediaSrc(block, data)
      if (!src) return ''
      const caption = (data.caption || []).map((c) => c.plain_text).join('')
      return `<figure class="my-10">
        <img src="${escapeAttr(src)}" alt="${escapeAttr(caption)}" class="w-full h-auto rounded-xl transition-transform hover:scale-[1.02] duration-300" loading="lazy" referrerPolicy="no-referrer" />
    ...
    case 'video': {
      ...
      if (!url) return ''
      const caption = (data.caption || []).map((c) => c.plain_text).join('')
      if (isSoundCloudUrl(url)) return renderSoundCloudEmbed(url, caption)
      ...
      return `<div class="my-10">
        <video controls class="w-full h-auto rounded-lg ">
          <source src="${escapeAttr(blockMediaSrc(block, data))}" type="video/mp4">
          您的浏览器不支持视频播放。
        </video>
      </div>`
    }
    ...
    case 'audio': {
      ...
      return `<figure class="my-8">
        <audio controls class="w-full">
          <source src="${escapeAttr(blockMediaSrc(block, data))}">
          您的浏览器不支持音频播放。
        </audio>
        ${caption ? `<figcaption class="text-center text-muted-foreground text-sm mt-3">${escapeHtml(caption)}</figcaption>` : ''}
      </figure>`
    ...
    case 'file': {
      ...
      return `<a href="${escapeAttr(src)}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 px-4 py-2 my-4 bg-muted/40 hover:bg-accent border border-border hover:border-primary/40 rounded-lg text-foreground/85 hover:text-primary/80 transition-colors">
    ...
    case 'pdf': {
      ...
      return `<div class="my-8">
        <iframe src="${escapeAttr(src)}" class="w-full rounded-lg border border-border " style="height: 600px;"></iframe>
      </div>`
    }
