...
// 缓存键按数据库隔离：主库用 database_v3 / categories_v3，资源库用 resources_v3 / resources_categories_v3
const cache = {
  database: null,
  categories: null,
  lastFetch: {
...
/**
 * 通用数据库拉取：按数据库 ID 查询 → 映射 → 过滤 Published → 缓存
 ...
 * @param {Array} [options.fallbackOnError] API 未配置/出错时返回的回退数据
 */
async function fetchDatabase(databaseId, { cacheKey, fillToThirty = false, fallbackOnError = [], props = envConfig.notion.properties }) {
  // 检查缓存
  ...
  if (!notion || !databaseId) {
    console.error(`❌ Notion API not configured for ${cacheKey}`)
  ...
  try {
    ...
    if (fillToThirty && posts.length < 30) {
...
/**
 * 通用分类提取：优先从已缓存的文章列表提取 → 查分类缓存 → retrieve 数据库 select 选项
 */
async function fetchCategories(databaseId, { postsCacheKey, categoriesCacheKey, fallback, categoryProp = envConfig.notion.properties.category }) {
  // 1. 尝试从缓存的文章列表中提取分类（最高效，且能避免 API 错误）
  if (isCacheValid(postsCacheKey)) {
    console.log(`✅ 从文章缓存中提取分类 (${postsCacheKey})`)
    const posts = cache[postsCacheKey]
  ...
  // 2. 检查分类缓存
  if (isCacheValid(categoriesCacheKey)) {
    console.log(`✅ 使用缓存数据 (${categoriesCacheKey})`)
    return cache[categoriesCacheKey]
  }
  ...
  try {
    ...
    if (categoryProperty?.type === "select") {
      ...
      setCache(categoriesCacheKey, categories)
      return categories
    }

    setCache(categoriesCacheKey, fallback)
    return fallback
  } catch (error) {
...
export async function getDatabase() {
  return fetchDatabase(FORMATTED_DATABASE_ID, {
    cacheKey: 'database_v3',
    fillToThirty: true,
    fallbackOnError: fallbackData,
  })
}

export const getCategories = async () => {
  return fetchCategories(FORMATTED_DATABASE_ID, {
    postsCacheKey: 'database_v3',
    categoriesCacheKey: 'categories_v3',
...
export async function getResourcesDatabase() {
  return fetchDatabase(FORMATTED_RESOURCES_DATABASE_ID, {
    cacheKey: 'resources_v3',
    fillToThirty: false,
    fallbackOnError: [],
...
export const getResourcesCategories = async () => {
  return fetchCategories(FORMATTED_RESOURCES_DATABASE_ID, {
    postsCacheKey: 'resources_v3',
    categoriesCacheKey: 'resources_categories_v3',
    fallback: [],
