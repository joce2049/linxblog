 ...
 * 供 /api/media 在请求时解析出「永不出现在缓存 HTML 里」的稳定 URL。
...
// 正文里会携带 S3 直链、需按块换新的媒体块类型
const MEDIA_BLOCK_TYPES = ['image', 'video', 'audio', 'file', 'pdf']

/**
 * 按 Notion 块 ID 实时取一个新鲜的媒体 URL（正文内嵌 图/视频/音频/文件/PDF）。
 * 同样靠 blocks.retrieve 每次重新签发的 S3 URL，供 /api/media 换新。
 * 外链媒体（external.url，如 YouTube/Unsplash）本就不过期，这里一并返回。
 * @param {string} blockId Notion 块 ID
 * @returns {Promise<string|null>} 新鲜的媒体 URL；非媒体块或未配置时为 null
 */
export async function getFreshBlockMediaUrl(blockId) {
  if (!notion || !blockId) return null

  const block = await notion.blocks.retrieve({ block_id: blockId })
  const data = MEDIA_BLOCK_TYPES.includes(block.type) ? block[block.type] : null
  if (!data) return null
  return data.file?.url || data.external?.url || null
}

/**
 * 一次性拉取「主库 + 资源库」所有页面的新鲜封面 URL，返回 pageId -> url 映射。
 * 供 /api/media 批量解析封面：一页几十张封面用一次（每库一趟）query 换取全部新鲜 URL，
 * 而不是逐图 pages.retrieve —— 那样会瞬间打满 Notion 的 ~3 次/秒限流、导致部分封面解析失败。
 * @returns {Promise<Map<string,string>>} pageId -> 新鲜封面 URL（无封面的页面不收录）
 */
export async function getFreshImageUrlMap() {
  const map = new Map()
  if (!notion) return map

  const targets = [
    { id: FORMATTED_DATABASE_ID, imageProp: envConfig.notion.properties.image },
    { id: FORMATTED_RESOURCES_DATABASE_ID, imageProp: envConfig.notion.resourcesProperties.image },
  ]
  ...
  for (const target of targets) {
    ...
    while (hasMore) {
      ...
      for (const page of response.results) {
        const url = extractImageUrl(page.properties, target.imageProp)
        if (url) map.set(page.id, url)
      }
      hasMore = response.has_more
      startCursor = response.next_cursor
    }
